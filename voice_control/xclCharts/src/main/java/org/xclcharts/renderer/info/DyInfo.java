...
public class DyInfo {
	...
	protected void drawInfo(Canvas canvas) //hint  ToolTips
	{
		...
		}else if(XEnum.DyInfoStyle.CAPLEFTRECT == mStyle){
			renderCapLeftRect(canvas,mRect);
	...
	//画带左箭头的矩形背景框
	private void renderCapLeftRect(Canvas canvas,RectF rect){

		if( !mShowBackground && !mShowBoxBorder)return;
		//取矩形高度的五分之一
		float AngleH = rect.height() * mScale; //0.2f ;
		rect.left += AngleH;
		rect.right += AngleH;
...
//		float AngleY = rect.bottom;
		...
		//取矩形中间位置下移十分之一
		path.lineTo( AngleX, centerY+AngleH/2);
		//箭头位置的点，矩形中间下移五分之一
		path.lineTo( AngleX - AngleH, centerY + AngleH );
		path.lineTo( AngleX, centerY - AngleH/2);
...
//		if(mShowBoxBorder)canvas.drawPath(path, this.getBorderPaint());
	...
	/**
	 * 返回坐标矩形，上一次点击后生成过的矩形。必须是点击一次生成后才会返回矩形，否则各坐标值为空
	 */
